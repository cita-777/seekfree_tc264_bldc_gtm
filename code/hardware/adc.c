...
void battery_check(void)
{
    ...
    {
        // 只有电压正常时才检查check_count
        if (adc_information.battery_voltage < 9.0)
        {
            if (adc_information.battery_voltage < (BATTERY_PROTECT_VALUE * 2))
            {
                check_count++;
            }
            else
        ...
        {
            if (adc_information.battery_voltage < (BATTERY_PROTECT_VALUE * 3))
            {
                check_count++;
            }
            else
        ...
        {
            if (adc_information.battery_voltage < (BATTERY_PROTECT_VALUE * 6))
            {
                check_count++;
            }
            else
        ...
        // check_count的状态设置移到这里，只在电压正常时执行
        if (check_count == 0)
        {
    ...
    {
        // 电压过低时，直接设置无电池状态，不再被覆盖
        // printf("电压过低，设置为无电池状态: %.2fV < 5.0V\n", adc_information.battery_voltage);
